...
`create table <table name> (<column name > <data type> [<size>] <constraints of you want>)`
...
#### (A.1) Inserting data for all the columns into a table:-
...
`INSERT INTO <Table name> VALUES (value1 for column1, value2 for column2, value3 for column3, ...);`

For example:

INSERT INTO student VALUES (Ravi, 26, 87, 12);

#### (A.2) The second method is from specific both the column names and the values to inserted.
Syntax--
...
INSERT INTO student (Name, Rollno, Marks, Class) VALUES (Rajesh, 2, 75, 12);
...
`INSERT INTO <Table name> (column1, column3, column7)VALUES (value1, value3, value7 ...);`
...
`INSERT INTO <Table name> (column1, column3, column7, column4) VALUES (value1, NULL, value7, NULL ...);`
For example:
...
`UPDATE <table name> SET <column 1>=<new value>,<column 3 >=<new value>,<column 5>=<new value> WHERE <condition>;`
...
`UPDATE <table name> SET <column 1>=<new value> + 1000 WHERE <condition>;`
...
`ALTER TABLE <table name> CHANGE <old Column name> <new column name> <datatype [size]>;`
...
|Bell   |CAT     |   F  | 2013-05-09  |  JATIN  | CHENNAI |
...
`SELECT * FROM pet WHERE sex = ‘m’͖`

2 = Select all pets that were born on or after Jan 10, 2016.

`SELECT * FROM pet WHERE birth >= '2016-1-10’͖`

3 = Select all female-dogs.

`SELECT * FROM pet WHERE sex = 'f' AND species = ‘dog’͖`

4 = Select all snakes or birds.

`SELECT * FROM pet WHERE species = 'snake' OR species = ‘bird’͖`
...
`Select <columnname> AS [column alias] [, <columnname> AS [column alias]] ͙.. From <tablename>;`
...
`SELECT owner, name FROM pet WHERE birth BETWEEN ‘2014-01-01’ AND ‘2019-01-01’;`
...
`SELECT * FROM pet WHERE city IN (‘DELHI’, 'MUMBAI', 'CHENNAI', ‘ GR  ’)͖`

ͻ The NOT IN operator finds rows that do not match in the list. So if you write.
`SELECT * FROM pet WHERE city NOT IN (‘DELHI’, 'MUMBAI', 'CHENNAI', ‘  GR  ’)͖`
